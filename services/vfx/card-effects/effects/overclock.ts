import type { CardFxDefinition, CardPersistentFxSource } from '../types';

const sumIntensity = (sources: readonly CardPersistentFxSource[]): number =>
  sources.reduce((s, src) => s + (src.intensity ?? 1), 0);

export const overclockCardFx: CardFxDefinition = {
  kind: 'overclock',
  className: 'card-fx-overclock',
  defaultPalette: { primary: '#ffaa00', secondary: '#00ccff', accent: '#882200' },
  aggregate(sources) {
    return {
      renderMode: sources.length <= 2 ? 'stacked' : 'aggregated',
      vars: {
        '--card-fx-count': String(sources.length),
        '--card-fx-intensity': String(sumIntensity(sources)),
        '--card-fx-primary': sources[0]?.palette?.primary ?? '#ffaa00',
        '--card-fx-secondary': sources[0]?.palette?.secondary ?? '#00ccff',
        '--card-fx-accent': sources[0]?.palette?.accent ?? '#882200',
      },
    };
  },
};
