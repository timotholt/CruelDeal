...
export const createCardVfxRegistry = (): CardVfxRegistry => {
  const GRACE_MS = 120;

  let counter = 0;
  let disposed = false;
  const uid = () => `vfx-${++counter}-${Date.now()}`;
  ...
  return {
    createTransient(request: CreateTransientVfxRequest): string | null {
      ...
      schedule(() => removeTransient(cardId, id), durationMs + exitDurationMs + GRACE_MS + 50);
    ...
    reconcilePersistent(cardId: string, sources: CardPersistentFxSource[]): void {
      ...
      for (const [kind, kindSources] of byKind) {
        ...
        if (kindSources[0]?.palette) {
          ...
          if (kindSources[0].palette.secondary)
            vars['--card-fx-secondary'] = kindSources[0].palette.secondary;
          if (kindSources[0].palette.accent)
            vars['--card-fx-accent'] = kindSources[0].palette.accent;
        }
    ...
    getLayers(cardId: string): CardVfxRenderModel {
      if (disposed) return { transient: [], persistent: [] };
      const s = cards.get(cardId);
    ...
    dispose(): void {
      ...
      timeouts.clear();
      for (const state of cards.values()) state.listeners.clear();
      cards.clear();
      dedupeKeys.clear();
    },
  };
