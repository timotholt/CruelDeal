...
const buildState = (
  ...
  for (const s of specs) {
    ...
    const inst: CardInstance = {
      ...
      revealed: zone === 'LANE',
      powerDelta: 0, tags: [], textOverride: null, counters: {},
  ...
  const eMap = opts.energy ?? { PLAYER: 3, OPP: 3 };
  return {
    turn: 3, maxEnergy: { PLAYER: 3, OPP: 3 }, nextTurnEnergyBonus: { PLAYER: 0, OPP: 0 },
...
// -- Respects energy budget -------------------------------------------------
{
  ...
  const state = buildState([], { energy: { PLAYER: 0, OPP: 3 } });
  const plays = planEnemyTurnFromPool(state, 'OPP', manifest, createRng('budget'));
...
// -- Skips unaffordable cards -----------------------------------------------
{
  ...
  const state = buildState([], { energy: { PLAYER: 0, OPP: 3 } });
  const plays = planEnemyTurnFromPool(state, 'OPP', manifest, createRng('nope'));
...
// -- No plays when energy is 0 ----------------------------------------------
{
  ...
  const state = buildState([], { energy: { PLAYER: 0, OPP: 0 } });
  const plays = planEnemyTurnFromPool(state, 'OPP', manifest, createRng('noenergy'));
...
// -- Respects lane capacity -------------------------------------------------
{
  ...
  // Fill lane 0 and 1 to cap. Only lane 2 has room for OPP.
  const specs: CardSpec[] = [];
  for (let i = 0; i < 4; i++) specs.push({ def: 'cheap', owner: 'OPP', lane: 0, zone: 'LANE' });
  for (let i = 0; i < 4; i++) specs.push({ def: 'cheap', owner: 'OPP', lane: 1, zone: 'LANE' });
  const state = buildState(specs, { energy: { PLAYER: 0, OPP: 6 } });
  const plays = planEnemyTurnFromPool(state, 'OPP', manifest, createRng('cap'));
...
// -- Ignores disabled cards -------------------------------------------------
{
  ...
  const state = buildState([], { energy: { PLAYER: 0, OPP: 6 } });
  const plays = planEnemyTurnFromPool(state, 'OPP', manifest, createRng('disabled'));
...
// -- Determinism ------------------------------------------------------------
{
  ...
  const state = buildState([], { energy: { PLAYER: 0, OPP: 5 } });
  const a = planEnemyTurnFromPool(state, 'OPP', manifest, createRng('same'));
  const b = planEnemyTurnFromPool(state, 'OPP', manifest, createRng('same'));
  eq(a, b, 'pool: same seed → identical plan');
  const c = planEnemyTurnFromPool(state, 'OPP', manifest, createRng('different'));
...
// -- maxPlays cap -----------------------------------------------------------
{
  ...
  const state = buildState([], { energy: { PLAYER: 0, OPP: 100 } });
  const plays = planEnemyTurnFromPool(state, 'OPP', manifest, createRng('cap'), { maxPlays: 3 });
...
// -- Custom fork tag changes stream -----------------------------------------
{
  ...
  const state = buildState([], { energy: { PLAYER: 0, OPP: 2 } });
  const rng = createRng('base');
  const p1 = planEnemyTurnFromPool(state, 'OPP', manifest, rng, { forkTag: 'tag-1' });
  const p2 = planEnemyTurnFromPool(state, 'OPP', manifest, rng, { forkTag: 'tag-2' });
...
// -- Plays cards from hand in cost order ------------------------------------
{
  ...
  const state = buildState(
    [
      { id: 'h1', def: 'big',   owner: 'OPP', lane: null, zone: 'HAND' },
      { id: 'h2', def: 'mid',   owner: 'OPP', lane: null, zone: 'HAND' },
      { id: 'h3', def: 'cheap', owner: 'OPP', lane: null, zone: 'HAND' },
    ],
    { energy: { PLAYER: 0, OPP: 3 } },
  );
  const plays = planEnemyTurnFromHand(state, 'OPP', manifest, createRng('hand'));
  // Total cost <= energy
...
// -- Respects lane capacity -------------------------------------------------
{
  const manifest = mkManifest([mkCard('cheap', 2, 1)]);
  const specs: CardSpec[] = [
    { id: 'h1', def: 'cheap', owner: 'OPP', lane: null, zone: 'HAND' },
  ];
  // Fill all 3 lanes for OPP.
  for (let lane = 0; lane < 3; lane++) {
    for (let i = 0; i < 4; i++) specs.push({ def: 'cheap', owner: 'OPP', lane: lane as LaneIdx, zone: 'LANE' });
  }
  const state = buildState(specs, { energy: { PLAYER: 0, OPP: 3 } });
  const plays = planEnemyTurnFromHand(state, 'OPP', manifest, createRng('full'));
...
// -- Determinism ------------------------------------------------------------
{
  ...
  const state = buildState(
    [
      { id: 'h1', def: 'a', owner: 'OPP', lane: null, zone: 'HAND' },
      { id: 'h2', def: 'b', owner: 'OPP', lane: null, zone: 'HAND' },
      { id: 'h3', def: 'c', owner: 'OPP', lane: null, zone: 'HAND' },
    ],
    { energy: { PLAYER: 0, OPP: 3 } },
  );
  const a = planEnemyTurnFromHand(state, 'OPP', manifest, createRng('same'));
  const b = planEnemyTurnFromHand(state, 'OPP', manifest, createRng('same'));
...
// -- Empty hand --------------------------------------------------------------
{
  ...
  const state = buildState([], { energy: { PLAYER: 0, OPP: 3 } });
  const plays = planEnemyTurnFromHand(state, 'OPP', manifest, createRng('empty'));
