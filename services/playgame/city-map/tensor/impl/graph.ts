...
import * as isectLib from 'isect';

const isect = isectLib as unknown as {
    bush: (lines: Array<{from: Vector; to: Vector}>) => { run: () => Array<{point: Vector; segments: Array<{from: Vector; to: Vector}>}> };
};
...
export default class Graph {
    ...
    private getNodesAlongSegment(segment: Segment, quadtree: d3.Quadtree<Node>, radius: number, step: number): Node[] {
        ...
        for (let i = 0; i <= steps; i++) {
            const currentPoint = start.clone().add(differenceVector.clone().multiplyScalar(i / steps));
    ...
    private fuzzySegmentsEqual(s1: Segment, s2: Segment, tolerance=0.0001): boolean {
        if (s1.from.x - s2.from.x > tolerance) return false;
        if (s1.from.y - s2.from.y > tolerance) return false;
        if (s1.to.x - s2.to.x > tolerance) return false;
        if (s1.to.y - s2.to.y > tolerance) return false;
        return true;
    }
