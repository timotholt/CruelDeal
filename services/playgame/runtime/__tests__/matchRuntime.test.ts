...
function runtimeFixture(seed = 'phase1-checkpoint3-runtime'): MatchRuntime {
...
describe('createMatchRuntime', () => {
  it('commits the symmetric opening as a system transaction over canonical genesis', () => {
    ...
    expect(opening.events).toHaveLength(BOOTSTRAP_MANIFEST.constants.startingHandSize * 2);
    expect(opening.events.map((event) => event.type)).toEqual(
      opening.events.map(() => 'CARD_DRAWN'),
    );
  ...
  it('publishes only after state, revision, receipt transaction, and the complete timeline commit', async () => {
    const runtime = runtimeFixture();
    ...
    expect(result).toMatchObject({ status: 'accepted', revision: baseRevision + 1 });
  ...
  it('turns malformed dequeue work into a typed receipt and continues draining', async () => {
    ...
    expect(runtime.transactions().at(-1)?.intent.intentId).toBe('after-malformed');
  });
  ...
  it('exports bootstrap, genesis, and a non-overlapping transaction log that folds to current state', async () => {
    ...
    expect(replayed).toEqual(runtime.state());
  });
  ...
  it('emits the resolution-start vocabulary before a legal turn cascade', async () => {
    ...
    runtime.subscribeCommittedTransactions((timeline) => {
      ...
      if (first.event.type !== 'TURN_RESOLUTION_STARTED') return;
