...
| Determinism and replay | `phase15-frame-continuity`, `replay-snapshot-parity.stress`, `gameplay-rng-authority`, runtime `frameReplay.edge`, `projection` | Framed history remains continuous; replay and live state agree; gameplay randomness is state-owned; projections do not disclose authority-only state. |
